...
if [[ $MUTE == *"MUTED"* ]]; then
    notify-send --replace-id=4 -i "$icon_path/volume-down.png" -e -t 1300 " Volume $VOLUME% $MUTE"
else
    notify-send --replace-id=4 -i "$icon_path/volume-down.png" -e -t 1300 " Volume $VOLUME%"
fi
